...
def geraRelatorio(il):
  ...
  for k, v in df.iterrows():
    ...
    if v["Nome"] == "Carteira":
      ...
      res["p"].append(
        "{:.0f}%".format(tot / (tot_carteira - tot_bens - tot_outros) * 100)
      )
  ...
  for b in [
    ...
    acct_list = ["Carteira -> {}".format(b[0]),"Investimentos -> Caixa -> {}".format(b[0])]
    for acct in acct_list:
      ...
      acct_list = ["Carteira -> {}".format(b[0]), "Investimentos -> Caixa -> {}".format(b[0]), "TOTAL {}".format(b[0])]
    ...
    mtm_acum = []
    id_cx = df.index[
      df["Nome"] == "Investimentos -> Caixa -> {}".format(b[0])
    ].tolist()[0]
    ...
    if sum(cx) == 0:
      ...
      bloco["tot"].append(
        "{:0,.2f}%".format(100 * (sum(cx) + sum(mtm)) / -sum(cx))
      )
    blocos.append(bloco)
  ##########
  ##########
