...
def geraTabela(contas, cpiAdj):
    ...
    udate = date(datetime.now().year + a,
                 datetime.now().month + b, 1) - timedelta(days=c)
...
def geraRelatorioCrescimento(conta):
    ...
    for c in df:
        if c not in ["Nome"]:
            ...
            res['valores_avg'].append(
                sum(res['valores']) / len(res['valores']))
            ...
            res['valores_infl_avg12'].append(
                sum(res['valores_infl'][-12:]) /
                len(res['valores_infl'][-12:]))
