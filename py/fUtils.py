...
def geraRelatorio():
    ...
    acct_dist = [
        "Despesas -> Compras -> Casa", "Despesas -> Filhos -> Presentes",
        "Despesas -> Lazer -> Ferias", "Despesas -> Lazer -> Mensal",
        "Despesas -> Living -> Mercado", "Despesas -> Living -> Alimentacao",
        "Despesas -> Compras -> Pessoais", "Despesas -> Saude -> Mensal",
    ...
    for c in df:
        if c != 'Nome' and c != 'Lvl' and c != 'ToSort':
            ...
            df.at[3010,
                  c] = df.at[3011, c] + df.at[3012, c] + df.at[3013,
                                                               c] + df.at[3014,
                                                                          c]
...
def MontaTabelaResumo(mes,ano):
  ...
  r = sqlQuery("SELECT t1.id, t1.datahora, t1.texto, t1.id_conta, t1.valor FROM Despesas t1 where cast(strftime('%Y', t1.datahora) as integer) = {} and cast(strftime('%m', t1.datahora) as integer) = {} order by t1.datahora, t1.texto".format(ano,mes))
  t = 0
  i = 0
  for l in r:
    t = t + l['valor']
...
def getProbLabel(nid):
  import string
  from database import sqlQuery
  texto = sqlQuery("SELECT texto from Despesas WHERE id = {} order by id".format(nid))[0]['texto']
  texto = texto.lower().translate(str.maketrans('', '', string.punctuation))
  texto = ''.join([i for i in texto if not i.isdigit()])
  to_predict = [texto]
  ...
  res = []
  for c in pbt:
    res.append({'Conta': c, 'Prob': pbt[c][0], 'Probs': "{:0,.2f}".format(pbt[c][0])})
...
def LabelTrain():
  ...
  import string
  from database import sqlQuery
  r = sqlQuery("SELECT t1.datahora, t1.texto, t1.valor, t2.conta, t2.id as id_conta FROM Despesas t1, Contas t2 where t1.id_conta = t2.id")
  i = 0
  for l in r:
    v = l['texto'].lower().translate(str.maketrans('', '', string.punctuation))
    r[i]['texto'] = ''.join([i for i in v if not i.isdigit()])
  ...
  X_train, X_test, y_train, y_test = train_test_split(text, labels, random_state = 0, test_size=0.01)
  count_vect = CountVectorizer()
  X_train_counts = count_vect.fit_transform(X_train)
  tf_transformer = TfidfTransformer().fit(X_train_counts)
  X_train_transformed = tf_transformer.transform(X_train_counts)
...
def geraRelatorio():
  ...
  for k,v in df.iterrows():
    ...
    if avg12 > avg:
      res['avg12'][-1] =  '<span style="color: green;">'+ res['avg12'][-1] + '</span>'
    elif avg12 < avg:
      res['avg12'][-1] =  '<span style="color: red;">'+ res['avg12'][-1] + '</span>'
