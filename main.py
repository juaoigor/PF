...
def investimentosCarteira():
  try:
    ...
    tb_res = sqlQuery("SELECT t2.bond, SUM(t1.qtde) AS qtde FROM BondsCarteira t1, Bonds t2 where t1.idbond = t2.id GROUP BY t2.bond HAVING SUM(t1.qtde) <> 0  ORDER BY t2.bond")

    return render_template("investimentos.carteira.html", modo=modo, tb=tb, rs=rs, bonds=bonds, tb1=tb1, tb_res=tb_res)
  except:
    exc_type, exc_value, exc_traceback = sys.exc_info()
    lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
    msg = "".join("\r\n<br>!! " + line for line in lines)
...
def investimentosFluxo():
  try:
    ...
    return render_template("investimentos.fluxo.html", tb1=tb1, tb2a=tb2a, tb2b=tb2b, ano=ano)
  except:
    exc_type, exc_value, exc_traceback = sys.exc_info()
    lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
    msg = "".join("\r\n<br>!! " + line for line in lines)
