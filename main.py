    ...
    try:
        ...
        if request.method == "GET":
            if request.args.get("mode") == "edit":
                ...
                eid = request.args.get("id")
                rs = sqlQuery(
                    "SELECT * FROM Contas WHERE id = {}".format(eid))[0]
...
def despesasEditar():
    ...
    if request.method == "GET":
        ...
        if "Apagar" in request.form:
            sql = "DELETE FROM Despesas WHERE ID = {}".format(
                request.form["id"])
            sqlExec(sql)
    ...
    if request.method == "GET":
        ...
        if "Transferir" in request.form:
            InsertValues("Despesas", [
                ...
                request.form["conta"], 1, 1, 1,
                request.form["datahora"], "{} (TRANSF: {})".format(
                    request.form["texto"], request.form["obs"]), "{}".format(
                        -1 * float(request.form["valor"]))
            ])
            InsertValues("Despesas", [
                "id_conta", "id_cartao", "id_bem", "id_pessoa", "datahora",
                "texto", "valor"
            ], [
                request.form["contaDestino"], 1, 1, 1,
                request.form["datahora"], "{} (TRANSF: {})".format(
                    request.form["texto"], request.form["obs"]), "{}".format(
    ...
    if request.method == "POST" and "Transfer" in request.form:
        if request.form["Transfer"] == "Transfer":
            ...
            InsertValues("Despesas", [
                ...
                request.form["contaOrigem"], 1, 1, 1,
                request.form["id_transfer"], request.form["data"],
                "(TRANSFER) {}".format(request.form["texto"]), "{}".format(
                    float(request.form["valor"]) * -1)
            ])
            InsertValues("Despesas", [
                "id_conta", "id_cartao", "id_bem", "id_pessoa", "id_transfer",
                "datahora", "texto", "valor"
