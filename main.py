...
def configAutoupdate():
  ...
  if request.method == 'GET':
    if request.args.get('mode') == 'edit':
      ...
      for l in r:
        sql = 'UPDATE Despesas set id_conta = {} WHERE texto like "{}" and texto not like "%EDITADO%"'.format(l['id_conta'],l['texto'])
        sqlExec(sql)

  elif request.method == 'POST' and 'Inserir' in request.form:
    InsertValues('autoupdate', ['texto', 'id_conta'], [ request.form['texto'], request.form['conta'] ])
  elif request.method == 'POST' and 'Update' in request.form:
    sql = "UPDATE autoupdate set texto = '{}', id_conta = {} where id = {}".format(request.form['texto'], request.form['conta'], request.form['id'])
    sqlExec(sql)
  ...
  tb = sqlQuery("SELECT t1.id, t1.texto, t1.id_conta, t2.conta FROM autoupdate t1, contas t2 where t1.id_conta = t2.id order by conta, texto")
  return render_template('config.autoupdate.html', labels=labels, modo=modo, tb=tb, rs=rs)
...
def configContas():
  ...
  if request.method == 'GET':
    ...
    sql = "UPDATE Contas set Conta = '{}', RecDes = {}, FixVar = {}, Inv = {} where id = {}".format(request.form['conta'], request.form['RecDes'], request.form['FixVar'], request.form['Invest'], request.form['id'])
    sqlExec(sql)
      ...
      if request.args.get('Filtrar') == "Filtrar":
        if request.args.get('texto') != "":
          if request.args.get('todos') != None:
            sqlFilt = "SELECT * FROM Despesas WHERE texto like '%{}%' ORDER BY datahora, abs(valor) desc LIMIT 50".format(request.args.get('texto'))
          else:
            sqlFilt = "SELECT * FROM Despesas WHERE id_conta = 0 and texto like '%{}%' ORDER BY datahora, abs(valor) desc LIMIT 50".format(request.args.get('texto'))
      ...
      for i in range(0,999):
        if 'c_{}'.format(i) in request.form:
          if int(request.form['c_{}'.format(i)]) != 0:
            sql = "UPDATE Despesas set id_conta = {} WHERE id = {}".format(request.form['c_{}'.format(i)], request.form['id_{}'.format(i)])
            from database import sqlExec
            sqlExec(sql)
...
def despesasClassNLPCat():
  if request.method == 'POST':
    if request.form['Update'] == 'Update':
      ...
      sql = 'SELECT id from Contas where Conta = "{}"'.format(request.form['conta'])
      id_conta = sqlQuery(sql)[0]['id']
      for i in range(0,999):
        if 'check_{}'.format(i) in request.form:
          sql = "UPDATE Despesas set id_conta = {} WHERE id = {}".format(id_conta, request.form['id_{}'.format(i)])
          sqlExec(sql)
...
def despesasEditar():
  ...
  if request.method == 'GET':
    ...
    if 'Apagar' in request.form:
      ...
      sql = 'UPDATE Despesas set id_conta = {}, texto = "{}" WHERE ID = {}'.format(request.form['conta'], request.form['texto'],request.form['id'])
      sqlExec(sql)
      return redirect(url_for(request.form['backTo']))
...
def despesasEditarContaMes():
    ...
    if request.form['Update'] == 'Update':
      ...
      for i in range(0,999):
        if 'id_{}'.format(i) in request.form:
          sql = 'UPDATE Despesas set id_conta = {}, texto = "{}" where id = {}'.format(request.form['conta_{}'.format(i)], request.form['texto_{}'.format(i)], request.form['id_{}'.format(i)])
          sqlExec(sql)
  ...
  return render_template('despesas.editarcontames.html', tb=r, labels=labels, id_conta=conta)

@app.route('/despesas/ignorados', methods=['GET', 'POST'])
def despesasIgnorados():
  from database import sqlQuery
  r = sqlQuery("SELECT * FROM Despesas t1 where t1.id_conta = 9999 order by t1.datahora")
...
def despesasNLP():
  ...
  if nid == 0:
    nid = sqlQuery("SELECT id from Despesas WHERE id_conta = 0 order by id")[0]['id']

  print(nid)
  linfo = sqlQuery("SELECT * from Despesas WHERE id = {}".format(nid))[0]
  lprox = sqlQuery("SELECT id from Despesas WHERE id_conta = 0 and id > {} order by id".format(nid))[0]
      ...
      for i in range(0,999):
        if 'id_{}'.format(i) in request.form:
          if int(request.form['conta_{}'.format(i)]) != 0:
            sql = "UPDATE Despesas set id_conta = {} WHERE id = {}".format(request.form['conta_{}'.format(i)], request.form['id_{}'.format(i)])
            sqlExec(sql)
...
def despesasResumoconta():
  ...
  if request.method == 'POST' and 'Salvar' in request.form:
    if request.form['Salvar'] == 'Salvar':
      ...
      for i in range(0,999):
        if '{}_ID'.format(i) in request.form:
          if int(request.form['{}_Conta'.format(i)]) > 0:
            sql = 'UPDATE Despesas SET id_conta = {}, texto = "{}" WHERE id = {}'.format(request.form['{}_Conta'.format(i)],request.form['{}_texto'.format(i)],request.form['{}_ID'.format(i)])
            sqlExec(sql)
      return redirect(url_for('relatorio'))
  ...
  tb = sqlQuery("SELECT * from Despesas WHERE id_conta = {} order by datahora".format(conta))
...
def relatorio():
  ...
  rel, contas, graph = geraRelatorio()

  return render_template('relatorio.html', rel=rel, contas=contas, graph=graph)
