...
def geraRelatorio(mes, ano):
  ...
  for categoria in categorias:
    ...
    contas = sqlQuery(
      "SELECT t3.Conta, t2.Risco FROM RiscosC t1, Riscos t2, Contas t3 where t1.idRisco = t2.id and t1.idConta = t3.id and t2.Categ  = '{}' and t2.Risco NOT IN ('Ignora')"
    ...
    cgat['vals'] = {
      k: v
      for k, v in sorted(
        cgat['vals'].items(), key=lambda item: item[1], reverse=True)
    }
...
def GeraRelatorioFCAno(ano):
  ...
  ipca = sqlQuery(
    ...
    .format(dt))[0]['valor']
  res = sqlQuery(
    "SELECT t1.bond, t1.startindex, t2.data, t2.tipo, t2.yield, t2.yieldper, t2.amtz, t3.qtde FROM Bonds t1, BondsFlows t2, BondsCarteira t3 WHERE t1.id = t2.idbond and t1.id = t3.idbond and strftime('%Y', t2.data) = '{}' order by t2.data"
    .format(ano))
